...
fun Container.buildSettings(settingsContainer: Container, menuContainer: Container) {
    settingsContainer.apply {
        ...
        labeledText(this, { S().colorTheme }, textSize = 22.0, x = 210.0, y = 275.0)
        val themeButtons = mutableListOf<SolidRect>()
        val themePreviewColors = listOf(
            RGBA(0x1a, 0x1a, 0x2e), RGBA(0x0a, 0x1a, 0x2e), RGBA(0x0a, 0x2e, 0x1a),
            RGBA(0x2e, 0x0a, 0x0a), RGBA(0x2e, 0x1a, 0x0a), RGBA(0x0a, 0x2e, 0x2e)
        )
        for (i in 0 until 6) {
            val bx = 120.0 + i * 70.0
            val btn = solidRect(55.0, 35.0, if (GameState.settings.themeIndex == i) RGBA(0xe9, 0x45, 0x60) else RGBA(0x30, 0x30, 0x40)) { position(bx, 305.0) }
            solidRect(40.0, 20.0, themePreviewColors[i]) { position(bx + 7.5, 310.0) }
            themeButtons.add(btn)
        }
        fun updateThemeButtons() { for (i in themeButtons.indices) themeButtons[i].color = if (GameState.settings.themeIndex == i) RGBA(0xe9, 0x45, 0x60) else RGBA(0x30, 0x30, 0x40) }
        updateThemeButtons()
        for (i in themeButtons.indices) themeButtons[i].onClick { GameState.updateSettings { themeIndex = i }; updateThemeButtons(); updateAllLabels() }

        labeledText(this, { S().firstTurn }, textSize = 22.0, x = 210.0, y = 355.0)
        val xBtn = solidRect(90.0, 45.0, RGBA(0xe9, 0x45, 0x60)) { position(200.0, 385.0) }
        text("X", textSize = 28.0, color = Colors.WHITE) { position(228.0, 392.0) }
        val oBtn = solidRect(90.0, 45.0, RGBA(0x30, 0x30, 0x40)) { position(310.0, 385.0) }
        text("O", textSize = 28.0, color = Colors.WHITE) { position(338.0, 392.0) }
        ...
        labeledText(this, { S().boardSize }, textSize = 22.0, x = 210.0, y = 445.0)
        val sizeButtons = mutableListOf<SolidRect>()
        for (sz in 3..5) {
            val bx = 190.0 + (sz - 3) * 90.0
            val btn = solidRect(75.0, 40.0, if (GameState.settings.boardSize == sz) RGBA(0xe9, 0x45, 0x60) else RGBA(0x30, 0x30, 0x40)) { position(bx, 475.0) }
            text("${sz}x${sz}", textSize = 18.0, color = Colors.WHITE) { position(bx + 14.0, 482.0) }
            sizeButtons.add(btn)
        }
        ...
        labeledText(this, { S().timer }, textSize = 22.0, x = 210.0, y = 530.0)
        ...
        for (i in timerOpts.indices) {
            val bx = 140.0 + i * 110.0
            val btn = solidRect(95.0, 35.0, RGBA(0x30, 0x30, 0x40)) { position(bx, 560.0) }
            val idx = i
            labeledText(this, timerLabels[idx], textSize = 14.0, x = bx + 8.0, y = 567.0)
        ...
        labeledText(this, { S().ai }, textSize = 22.0, x = 150.0, y = 610.0)
        val aiBtn = solidRect(120.0, 40.0, if (GameState.settings.aiEnabled) RGBA(0x0f, 0x34, 0x60) else RGBA(0x40, 0x40, 0x50)) { position(400.0, 605.0) }
        labeledText(this, { if (GameState.settings.aiEnabled) S().on else S().off }, textSize = 20.0, x = 430.0, y = 613.0)
        aiBtn.onClick { GameState.updateSettings { aiEnabled = !aiEnabled; if (aiEnabled) multiplayer = false }; aiBtn.color = if (GameState.settings.aiEnabled) RGBA(0x0f, 0x34, 0x60) else RGBA(0x40, 0x40, 0x50); updateAllLabels() }

        labeledText(this, { S().aiDifficulty }, textSize = 18.0, x = 150.0, y = 655.0)
        ...
        for (i in diffOpts.indices) {
            val bx = 140.0 + i * 110.0
            val btn = solidRect(95.0, 35.0, RGBA(0x30, 0x30, 0x40)) { position(bx, 680.0) }
            val idx = i
            labeledText(this, diffLabels[idx], textSize = 14.0, x = bx + 8.0, y = 687.0)
            diffButtons.add(btn)
        }
        ...
        val backBg = solidRect(200.0, 45.0, RGBA(0xe9, 0x45, 0x60)) { position(200.0, 730.0) }
        labeledText(this, { S().back }, textSize = 22.0, x = 268.0, y = 739.0)
